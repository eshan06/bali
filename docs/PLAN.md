...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  and its extensions), and B4a (the API accepts the phone's own app client
  beside the portal's: `AUTH_AUDIENCE` is a list); B4b is next, then
  B4c.** Of the owner decisions Phase 3
...
- **An unlock made before the tap's answer is filed under its tap** (owner
  ...
  `docs/DECISIONS.md`.
- **Owner action for B4 (AWS console): the phone's sign-in** (B4a ✅). The
  dev pool has no hosted-UI domain and no app client for the phone yet:
  `docs/DEPLOY.md`, "The phone's sign-in (dev)" — the domain, the phone's
  public client (`bali://auth/callback`) and a refresh-token expiration
  above Cognito's 30 days, at which every student would be signed out
  monthly. Then hand the planning session the domain and the client id:
  they go in `ios/project.yml` (B4c) and, after the id already there, in
  dev's `AUTH_AUDIENCE`, which B4a made a list.
