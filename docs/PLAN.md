...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  and B1c (the API client) — B2 (the app skeleton and its macOS CI) and
  B3a (the outbox store and its retry bound); B3b is next.** Of the
...
- **B3b** The sync engine: the loop draining `nextDue` through one shared `APIClient` (B1c: a response's `result` and `answer` go straight to `settle`; on `.networkError`, `noAnswer` says back off, `.unreachable`, or wait on sign-in, `.noToken`), the 30-second check-in, and applying answers — the reconcile, reads stamped with `awaiting()` (`readMayReconcile`), and the unlock guard (`holdsUnlock`). Also its: a retry-now for the student's retry and after a reauth; GRDB's multi-process setup (a busy timeout, suspension in the background) before an extension opens the database (B5); and what an unlock made while the phone's own tap is unanswered is sent to — it has no session yet (with B6) · **B4** Cognito PKCE sign-in — B1c's `TokenProvider`: nil when there is no token right now, which is never a sign-out; refreshing after a `reauth` is B4's
