...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  words from the kept standing) and B6a (the NFC tap, and decision 11
  on the phone); B6b is next.**
...
- **The NFC tap, and decision 11 on the phone** (B6a ✅, 2026-09-25): a
  ...
  answer, a tag that is not a block, a scan cancelled. **To know:** an
  unlock under a re-tap of *another* teacher's block (it only arms) is
  kept with no class, and the class the phone was in shields it again —
  decision 11 files an unlock where its tap lands; the next Emergency
  Unlock goes to that class. Over an unread standing, Emergency Unlock is
  B6b's (next): with no tap pending there is none yet, and one under a
  pending tap leaves the last run's shields on. Why: `docs/DECISIONS.md`.
...
- **B6b** Emergency Unlock over an unread standing (#92's review) — the engine's path over a standing the file will not give back: the shields drop locally at once; the unlock is recorded and never discarded — under the pending tap's `event_id` when the queue holds one (B6a routes it so already; B6b drops the last run's shields for it), otherwise kept and sent once the standing reads or the server names the session (and decided: where it goes when the truth names none). Decide and document whether a fail-safe ceiling also applies to shields the phone can no longer justify. The Debug readout gets the button; C5 draws the screen
...
11. An emergency unlock made while the phone's own tap is still unanswered —
    ...
    and it guards every session until that answer names one.
