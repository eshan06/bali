...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  and late records) landed, and so has all of B1 — B1a (BaliCore's wire
  ...
  (the outbox store and its retry bound) and B3b-1 (the sync engine's
  drain); B3b-2 is next.** Of the owner decisions Phase 3
  needs (items 6–11 under Open product decisions), 7–10 are decided
  (2026-09-24), and 6 and 11 are open;
...
- **B3b-2** The check-in and the reconcile: the 30-second check-in while the app is in the foreground; applying answers to the phone's truth — a change's own answer, and reads (the check-in, `GET /v1/me`) stamped with `awaiting()` (`readMayReconcile`), with the unlock guard (`holdsUnlock`) — observable for C1–C6 and B5; and GRDB's multi-process setup (a busy timeout, suspension in the background, persistent WAL) before an extension opens the database (B5) · **B4** Cognito PKCE sign-in — B1c's `TokenProvider`: nil when there is no token right now, which is never a sign-out; refreshing after a `reauth` is B4's, through `SyncEngine`'s `refresh` seam (true once a fresh token is ready: the engine then sends everything again at once), with `retryNow()` once a sign-in gives the provider a token
- **B5** Enforcement: shields (every app a third party can block — `.all()` apps and web domains, no picker), session schedule, monitor extension, custom shield — 📱 settles Phase 0's open question. A tap the server has not answered yet schedules the 50-minute cap of decision 7. Note: "only a re-tap leaves protection off" holds per participation, not per phone — an armed tap converted at another teacher's Start joins that session focused, so the phone must re-report protection off there at its next check-in. The seams are B2's: `SessionMonitor` (`ios/BaliMonitor`) — `intervalDidEnd` clears the shields — and `ShieldConfigurationExtension` (`ios/BaliShield`), whose one `shield()` draws the custom shield; each `// B5:` note says what goes there
- **B6** NFC tap → local record → shield → outbox — 📱 The app's NFC entitlement and usage string are in (B2); v2 read the block with `NFCNDEFReaderSession`. An emergency unlock made while that tap is still unanswered has no session to send to yet: open decision 11, settled first
...
11. An emergency unlock made while the phone's own tap is still unanswered —
    what it is sent to. **Open, before B6 and C5.** The unlock endpoint needs
    a session id, and the phone has none until the tap is answered. The
    unlock contract settles a tap whose answer names a session (send it there,
    guarding that session until recorded), but not one answered armed (no
    session exists yet), recorded but no longer current (the answer names
    none), or refused (there never is one) — and naming the session the phone
    was in before the tap is wrong once the tap switches it: the unlock lands
    in the old session, and a read then re-shields the new one over it. Lean:
    the phone binds the unlock to its tap — queued behind it, acted on at
    once, guarding whatever session the tap's answer names — and the API gains
    `POST /v1/taps/{eventId}/unlock` (additive, an A-step), recording it
    against the session that tap was recorded in (late or noted, as today), or
    with no session under a note when the tap is still armed or never landed.
    Until decided, `SyncEngine.record` needs a session, so nothing sends one.
