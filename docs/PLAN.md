...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  fixtures), A6 (the join-code preview), A7 (the student's history) and A8
  (edit own name, unique within each class) landed; A9 is next.** Of the
...
- **B1** `BaliCore` Swift package (types, API client, the outbox dispositions, fixture contract tests) + a Linux Swift CI job. The fixtures are `contracts/fixtures/` (A5): each file names its type and the disposition the TS table gives, so every one must decode and every port must agree; decode `error.reason` leniently — a value BaliCore does not know is none
