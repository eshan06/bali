...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  words from the kept standing), B6a (the NFC tap, and decision 11
  on the phone), B6b (Emergency Unlock over an unread standing, and
  a scan that joins no class — the owner's ruling) and B6b-2 (#95's
  review: the unlock guard on every answer, and an unlock not filed yet
  filed from the file); B6c is next.**
...
- **Bali's own shield** (B5c ✅, 2026-09-25): over a blocked app,
  ...
  the bell cannot be known: never a time it cannot know. **OK** closes the app;
  ...
  (round 3's step 1 checks it), and the follow-up is the app writing the
...
- **B5c** The custom shield — `ShieldConfigurationExtension.shield()` (`ios/BaliShield`) shows "Focused with Bali until 9:42", the end read from the standing kept in the app group, in D1's approved look (light, the ring mark); nothing new designed. 📱 the shield's words over a blocked app — ✅ `ShieldWords` (`ios/BaliOutbox`) holds the words, from the standing and the queue the app keeps, read through B5b's bounded open (the whole open and read within 2 s, SQLite's locks included; the file closed before it returns): focused in a session, "Focused with Bali until 9:42", in the phone's own short time (its locale, 12- or 24-hour as set, its zone: "9:42 AM", "09:42"); a tap not yet answered keeping the shields on (`SyncState.tapHeldUntil`, the rule `shieldedUntil` already used), "Focused with Bali — waiting for your class", no time — its answer may move the bell; anything else — the file not read, the bell past, nothing keeping them on — "Focused with Bali", never a time it cannot know; beneath, D1's line ("This app is paused for class…", "This website…" over a website). The extension carries D1's look as far as `ShieldConfiguration` goes: stone-50 over a light material, the ring mark without its tile (the artboard's SVG, a vector asset, never tinted), text-primary and text-secondary, and **OK** in white on green-700 — with no shield action extension, iOS's own action: the blocked app closes; the shield cannot open Bali, where Emergency Unlock is (C5). Not carried: D1's font, sizes and layout, which are iOS's. It links BaliOutbox, as the monitor does. Tests: `ShieldWordsTests` (Linux and the iOS Simulator), `AppTests.mark` (the simulator: the extension ships the mark, drawn as D1 has it); of 12 mutations of the rules, all 12 turn a test red
  - 📱 **Round 3 — the owner's, after B5c merges** (`ios/README.md`, "Round 3 (B5c)", has each step's expected result), after rounds 1 and 2 on the same build, `npm run dev:teacher -- watch` running:
    1. `start 20`, **Tap**, a blocked app: Bali's shield — light, the ring mark, "Focused with Bali until" the bell, D1's line; **OK** closes the app. `extend 10`, a check-in: the new bell (if the old one is still up, iOS keeps a shield's words until the shields change — a follow-up).
...
- **B6** NFC tap → local record → shield → outbox — 📱 The app's NFC entitlement and usage string are in (B2); v2 read the block with `NFCNDEFReaderSession`. An emergency unlock made while that tap is still unanswered is filed under the tap (decision 11): sent with the tap's `event_id` to A11's `POST /v1/taps/{eventId}/unlock` through BaliCore's `APIClient.unlock(tap:_:)`, answered as any unlock (`unlockDisposition`), and always sent there, even once the tap's answer names a session: a retry is answered where it was recorded, and the session route would refuse an id another session holds. Its tap, answered later, may say `joined` and `unlocked` — the unlock filed then: `apply_session`, no shield. Its request carries the record's order like every other outbox record (A12: `OutboxRecord.request`), so the server orders it after its tap whatever the clock says. Ships in three for size (B6a–B6c, below)
- **B6a** The NFC read and decision 11 on the phone, and #92's `firstMatch` rider — ✅ **What identifies a block:** the code written on it, not the chip — ten letters and digits, as v2's teacher app wrote every block (`T7XK2M9QPF`), in the tag's NDEF message: a well-known Text record holding it, or a URI record ending in it (`bali://t/<code>`, `https://<host>/t/<code>`, v2's QR link). Read whatever its case and surrounding space, sent upper-case: the `tagId` `POST /v1/blocks` registers and `POST /v1/taps` resolves. Any other tag — a link, other text, a blank tag — is not a Bali block: said so, nothing recorded. The rule is `BlockTag` (BaliCore, pure: the Text and URI records read from their bytes, UTF-8 and UTF-16); the reader, `BlockReader` (the app), a thin adapter over `NFCNDEFReaderSession` as v2's was, answering `BlockRead` — a block, not a block, cancelled, a phone with no NFC, iOS's error. A block read is the tap (`SyncEngine.tap(_:)`): recorded with its order (A12), shielded at once to decision 7's cap, sent. **Decision 11:** `SyncEngine.emergencyUnlock(reason:)` files it under the latest tap not yet answered (`Change.unlockUnderTap`), else in the session the phone is in (`SyncState.emergencyUnlock`); under a tap it acts at once — the tap's hold ends, the session the phone stood in stands unlocked — and goes to `POST /v1/taps/{eventId}/unlock` always, retries included, with its order, answered as any unlock. Until its tap is answered it guards every session — any may be the tap's; a refused tap, stuck, keeps it so (#94's Claude Review) — then the one the answer names (`settle` hands it on: `holdsUnlock`), none if it names none; its tap's answer does not apply while it waits, so nothing shields between the two answers — and, stuck, it keeps that answer's `focused` from shielding too (santa's review; by the phone's order, so a re-tap after it still returns to focus). The outbox file's migration `v3` makes the table again for it (`tapId`; an unlock under a tap names no session until the answer does) and carries its AUTOINCREMENT counter over — dropped with the table, it started again at 1, and A12's order with it. The Debug readout: **Scan**, **Read block code** (read only, to register a block), the typed **Tap** kept for rounds 1–3, Emergency Unlock through the engine (B5a's disclosure closed: it named the session even while a tap was unanswered), `Outbox:` and **History**. Rider (#92's review): `AppConfigTests` pins each setting to be set once in `project.yml`. Tests: `BlockTagTests`, `UnlockRouteTests`, `UnderTapEngineTests`, the migration from `v2` (drained and not), the schema, the requests, `tap-unlock/*`'s fixtures and the generated unlock cases through the tap route, the random walk, the monitor's wake; `AppTests.noNFC` (the simulator); of 29 mutations of the rules, all 29 turn a test red
  - 📱 **Round 4 — the owner's, after B6a merges** (`ios/README.md`, "Round 4 (B6)", has each step's expected result), after rounds 1–3, on B6a's build:
    1. **Register your block:** **Read block code**, the block held to the phone's top: `block <CODE>: read only, nothing recorded`, `Outbox: empty`; then `npm run dev:teacher -- block <CODE>` on the Mac. (`not a Bali block`: nothing on it is a code — write one, ten letters and digits in a Text record, with an NFC writer app.)
    2. **A real Scan:** `start 20`, `watch`, **Scan**: `tap recorded`, `Standing: focused until` the bell, `shields on`, `Outbox: empty` once answered; `watch` focused; a blocked app shows Bali's shield.
    3. **Airplane Mode, Scan, then Emergency Unlock before the answer:** shields off at once, `Outbox: tap · unlock under its tap`; Airplane Mode off: within two minutes (the outbox's backoff) `Outbox: empty`, still unlocked, never shielded between; `watch` unlocked; **History** `unlock · tap_in · tap_in`.
    4. **A tag that is not a block** (a link, a blank sticker): `not a Bali block — nothing recorded`, `Outbox: empty`, nothing changed.
    5. **A scan cancelled:** `scan cancelled — nothing recorded`, `Outbox: empty`.
- **B6b** Emergency Unlock over an unread standing (#92's review), the owner's ruling on a scan that joins no class (2026-09-25, planned as B6d and folded in), and #94's two riders — ✅ **Over a standing the file will not give back** (`Standing.unread`), `SyncState.emergencyUnlock` files the press under the tap not yet answered, as B6a did, else as `Change.unlockUnfiled`: an unlock with no session and no tap, which outbox migration `v4` allows (the table made again as `v3` made it, its AUTOINCREMENT counter carried over — pinned, drained and not). Either takes the last run's shields off at once (`SyncState.unlockedLast`: the press has the last word — after the tap not yet answered, or not filed; one the last run left queued does not). Not filed, it is never sent (`nextDue`), holds nothing behind it, awaits no answer (`awaiting`), guards every session (`holdsUnlock`) and acts on where the phone stood, read at a relaunch or by the monitor (`Outbox.standing()`); once the phone knows where it stands — the file read again, or the server's truth — it is filed there in the write that keeps that standing (`Outbox.file`), and sent. **Decided:** naming no session, it goes under the phone's last tap (`lastTap`), which the server files where that tap landed, by that session's rules, or keeps with no class — never refused, no server change; with no tap known, it waits for a truth that names one. **Decided: no fail-safe ceiling** over the last run's shields: Emergency Unlock is their way out, always recorded; decision 7's cap stays on the shields the enforcer put on for a tap. The Debug readout's Emergency Unlock works over an unread standing (`Outbox:` shows `unlock, its class not known yet`). **The owner's ruling** (`Outbox.refile`): a press under a scan answered armed or refused — or kept with no class while its scan is stuck unanswered — is filed again in the session the phone stood in when the student pressed (kept on the press's row: rider 1), as a session unlock of its own with the press's reason, time and order (`orderSeq`, `v4`), so the server places it against a refocus or re-tap made after the press (A12, A13), and a refocus there waits for it; minted once, in the write that settles that answer; none for a scan that lands in a class — a refused one landing on a later retry gets both records. **Riders (#94's review):** (1) an unlock under a tap answered with no session no longer loses its guard — it keeps the session the phone stood in until recorded, or a return there after it by the phone's order (red first); (2) `AppTests.noNFC` is disabled where the phone reads NFC (`BlockReader.canRead`). Tests (Linux and the iOS Simulator): `UnreadUnlockTests`, `RefiledTests`, `SchemaTests.fromV3`, the route from an unread standing, the random walk with unlocks filed and followed up; of 25 mutations of the rules, all 25 turn a test red
  - 📱 **Round 4, step 6 — the owner's, after B6b merges** (`ios/README.md`, round 4): in a session, Airplane Mode, **Scan** a block no teacher registered, **Emergency Unlock**; Airplane Mode off: the scan refused (`Outbox: tap (stuck: 404)`), the shields never back on, and `watch` shows the student unlocked — the class's grid sees the unlock.
- **B6b-2** #95's two enforcement WARNs (its Claude Review, comment 5837915189) — ✅ **The unlock guard on every answer:** only a tap's answer was guarded (`holdsUnlock`), so an unlock's late answer (`recorded`, `superseded`, `focused`) — or a state change's `applySession` — shielded again over an unlock the server had never recorded: one not filed yet (B6b), or, on `main` before it, another stuck. Every answer that would set `focused` is guarded now, as reads are (`keepsUnlocked`), by the phone's order: a tap's or a refocus's own answer passes its `seq` (`Change.isReturn`), any other none — and, as for reads, a phone focused there already, by the student's return since, applies it. **Filed from the file:** `keepStanding` always files (`Outbox.file`, which says whether it filed one); a read of the file's standing writes it back, and so does a launch unless its own read of the queue said nothing waits; a change's own write files first (`Outbox.record`); and a read failing after a change keeps that change in the list. Tests: the reviewer's path, two unlocks one stuck (either landing late), a re-tap after both, a refocus's and protection off's answers, the press with its re-read failing, a launch that cannot read its queue (at launch, the file read again, the server), a filing refused then the student's next act — red first on `main`, each, but the two that pin what must not change (the re-tap after both, the refocus's answer). Of 17 mutations of the rules, 16 turn a test red; the one left, the guard's fail-safe when its own read fails, is `main`'s and shows only between two reads a moment apart
- **B6c** The extensions' read (#93's review, comment 5833779234) and the monitor's bookkeeping (#92's review) — a read-only `Outbox.read` for the shield and the monitor: a reading coordination, `configuration.readonly`, no migration when the schema is current (a file a newer build migrated still reads as unreadable, "Focused with Bali"; one this build has yet to migrate — the app not opened since an update, B6a's `v3` — decided there); its 2 s bound a hard ceiling — `Outbox.granted`'s second wait bounded, so iOS's synchronous `configuration(shielding:)` and the monitor never wait unbounded; and `SessionMonitor`'s refusal bookkeeping moved into `Bell` (`carryOut`) and tested on Linux — built in B6a, moved for size, kept on the branch `claude/fervent-bell-8nvjcp-b6c-monitor`
...
| Feature | Phase | Notes |
|---|---|---|
| Core loop: tap→shield offline, armed taps, live grid, unlock always-recorded, refocus, join codes, roster, removal, self-expiry | 1–2 | ✅ built. **On the phone (B6a ✅):** the block read over NFC — the code written on it — into the tap, shielded at once offline; an Emergency Unlock before the tap's answer filed under that tap (decision 11) — and again in the class the student stood in when the scan joins no class, the owner's ruling (B6b ✅); over a standing the phone cannot read, Emergency Unlock still takes the shields off, kept until it can be filed (B6b ✅) |
...
11. An emergency unlock made while the phone's own tap is still unanswered —
    ...
    before the tap is wrong once the tap switches it.) B6a ✅ sends it:
    `SyncEngine.emergencyUnlock` files it under a tap not yet answered,
    and it guards every session until that answer names one. **The
    owner's ruling (2026-09-25), built in B6b ✅:** a scan that joins no
    class — armed, or refused — has the unlock filed again in the class
    the student stood in: one press, two records.
