...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  and late records) and A10 (a late unlock recorded, never applied — the
  owner's ruling) landed, and so has all of B1 — B1a (BaliCore's wire
  ...
  (the outbox store and its retry bound) and B3b, the sync engine (B3b-1,
  its drain; B3b-2, the check-in, the reconcile and one file for the app
  and its extensions); B4 is next.** Of the owner decisions Phase 3
...
- **A late unlock is recorded, never applied** (owner, 2026-09-24; A10 ✅):
  ...
  the server can judge "after" only by the clamped device times, so a phone
  clock turned back between the student's tap and a real unlock makes that
  unlock read as late — recorded all the same, and answered with the focus
  the phone then shields to, so the grid is never green over an unshielded
  phone; but that student's unlock does not take until their clock is right
  (C5 says so). Closing even that needs the phone to send the order it acted
  in with the unlock — additive, not built. Why: `docs/DECISIONS.md`.
...
- **B6** NFC tap → local record → shield → outbox — 📱 The app's NFC entitlement and usage string are in (B2); v2 read the block with `NFCNDEFReaderSession`. An emergency unlock made while that tap is still unanswered is filed under the tap (decision 11): sent with the tap's `event_id` to A11's `POST /v1/taps/{eventId}/unlock`
- **C1–C6** Screens: onboarding · join + preview · home / waiting · focus · unlocked, protection off, session over · history + me. C5: an unlock answered `superseded` with no refocus or tap of the phone's since means its clock is behind — the shields come back; say why, and how to set the time right (A10)
...
11. An emergency unlock made while the phone's own tap is still unanswered —
    what it is sent to. **Decided 2026-09-24 — A11 builds the endpoint; B6/C5
