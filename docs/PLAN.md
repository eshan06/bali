...
- **Phase 3 is under way** (2026-09-23): the step list under Phases replaces
  ...
  and late records) landed, and so has B1a (BaliCore's wire types and the
  fixture contract tests); B1b is next.** Of the
...
- **BaliCore has landed** (B1a, 2026-09-24): `ios/BaliCore`, the iOS apps'
  ...
  (Linux)" CI job. **Owner's call:** whether to make that job a required
  check in `protect-main` — it runs on every PR, not path-filtered, so
  requiring it never leaves a PR waiting on a check that did not run.
...
- **B1b** The outbox tables in BaliCore: `tapDisposition`, `stateChangeDisposition` and `unlockDisposition` ported, and `readMayReconcile`; the contract test checks each fixture's `disposition` against the port
